...
function search1() {
  ...
  if (productSearch != "") {
    for (let i = 0; i < arrProduct.length; i++) {
      if (
        arrProduct[i].name.toLowerCase().search1(productSearch) != -1 &&
        productSearch != ""
      ) {
...
function showProductDetail(productName) {
  ...
  //   return num.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") + "đ";
  // }
  let product = JSON.parse(localStorage.getItem("product6"));
  const productSingle = document.querySelector(".product-single");
  for (let i = 0; i < product.length; i++) {
    if (product[i].name === productName) {
      const totalPrice = currency(product[i].price);
      productSingle.innerHTML = `
            ...
            <div class="modal-contents">
              ...
              <div class="modal-main">
                <img src="${product[i].image}" alt="" class="modal-image" />
                <div class="modal-content">
                  <span class="modal-title">${product[i].name}</span>
  ...
  function createUser(e) {
    ...
    if (fname.value.length === 0) {
      ...
      if (!ten_regex.test(fname.value)) {
        ...
        document.querySelector("#fname").style.border = "1px solid #e5e5e5";
